...
این ابزار به‌صورت **فایل اجرایی آماده (precompiled)** ارائه می‌شود و **نیازی به build یا Go ندارد**.
...
- پشتیبانی از 3 حالت اسکن:
  ...
  - **ICMP** (ping واقعی – نیازمند root یا `CAP_NET_RAW`)
...
1. **انتخاب نوع اسکن**
   ...
   - `ICMP` → نیازمند root ⚠️
        ...
        - یا ست کردن capability
            ...
            ```
            > اگر دسترسی وجود نداشته باشد، برنامه **خودکار به TCP fallback می‌کند**.
...
bgscan/
 ...
 ┣ scanner/   # مدیریت اسکن و concurrency
 ┣ main.go    # CLI و flow اصلی برنامه
```
